...
design_main <- function(n, bin_p2, mu3, sd3, mu4, sd4){
  
  ###
  ### Generate X1 Matrix and x1 vector
  ###
  X1.save <-  matrix(rnorm(n*n, 2, 1), n, n)
  diag(X1.save) <- NA
  ...
  ###
  X4.save <-  abs(matrix(rnorm(n*n, mu4, sd4), n, n))
  diag(X4.save) <- NA
...
for(iter in iter_num){  
  for(city_size in city_sizes){
    ...
    #'@_e_ij
    set.seed(iter)
    Error_term.example <- error_term_gene_truNormal( n = city_size, a1 = 0, b1 = Inf, mu1 = 0,
                                                     sigma2_1 = 1, a2 = 0, b2 = Inf, mu2 = 1,
                                                     sigma2_2 = 6, a0_vec = 0, b0_vec = 0,
                                                     mu0_vec = c(-1,1), pho_ab = 0.5,
                                                     sigma_a = 1, sigma_b = 1 )
    
    #'@_X
    set.seed((iter-1) %/% X_num)
    X.example <- design_main(n = city_size, 0.6, 1, 1, 1, 1)
    ...
    eta_list_4 <- c(0.133278190, 0.018031669, 0.1, 0.023326042, 0.003024561, 0)
    ...
    #'@_Var(beta)_oracle
    eta_list_3 <- c(0.133278190, 0.018031669, 0.1, 0.023326042, 0.003024561, 0)
    ...
    write.table(write.out, file = paste("setting2.csv",
